...
def check_rtf(file_path):
    ...
    for i in rtf_tags:
        ...
        break
    if flag:
        debug_print("RTF conforms to schema")
    else:
        debug_print("Does not adhere to schema")
    return flag
...
- Test case step : Verify that the file name contains a '.rtf' extension. If not, write test cases for other file types
...
def test_check_rtf():
    try:
        with open('/Users/adithi/Desktop/jenkins_folder/L-16-01-07-random.rtf', 'r') as file:
            ...
            actual_output = check_rtf(rtf_content)
            assert actual_output == expected_output
    except:
        print(f"AssertionError: Does not adhere to schema ")
...
def test_extract_header():
    try:
        ...
        print(f"AssertionError: Header Not Found")
...
def test_extract_table_data():
    try:
        ...
        print(f"AssertionError: Table data not found")
